...
public class UserInfoUtils {
    ...
    public static void setPlayerName(String UUID, String newName) {
        JedisTempStorage.AddCommandToQueue_INNER("update:=:uuidname:=:add:=:" + UUID + ":=:add:=:" + newName.toLowerCase(Locale.ROOT));
    }
    public static void setPlayerUUID(String newName, String UUID) {

        String oldname = UserInfoStorage.uuidname.get(UUID);
        if(!newName.equalsIgnoreCase(oldname)) {
            JedisTempStorage.AddCommandToQueue_INNER("update:=:nameuuid:=:add:=:" + newName.toLowerCase(Locale.ROOT) + ":=:add:=:" + UUID);
        }
    }
    ...
    public static void setPlayerOrginName(String newName, String UUID) {
        if(UserInfoStorage.uuidname.containsKey(UUID)) {
            String oldname = UserInfoStorage.uuidname.get(UUID);
            if (!newName.equalsIgnoreCase(oldname)) {
                JedisTempStorage.AddCommandToQueue_INNER("update:=:namename:=:remove:=:" + oldname.toLowerCase(Locale.ROOT) + ":=:add:=:" + newName);
                JedisTempStorage.AddCommandToQueue_INNER("update:=:namename:=:add:=:" + newName.toLowerCase(Locale.ROOT) + ":=:add:=:" + newName);
            } else {
                JedisTempStorage.AddCommandToQueue_INNER("update:=:namename:=:add:=:" + newName.toLowerCase(Locale.ROOT) + ":=:add:=:" + newName);
            }
        } else {
